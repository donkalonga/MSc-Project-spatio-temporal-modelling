...
dat<-read.csv("Multitype_Model_Data.csv") %>% # 310 cases
  dplyr::select(hh_LAT, hh_LNG, lineage) # %>%
...
ttx <- xym$x[xym$marks == "major0" | xym$marks == "major1" | xym$marks == 
                   "major2" | xym$marks == "major3" | xym$marks == "major4" | xym$marks == 
                   "major5" | xym$marks == "major6"]
...
chooseCellwidth(xym,cwinit = CellWidth) # cell width is 175 metres
...
form_list <- formulaList(list(major0 ~ 1, major1 ~ 1, major2 ~ 1, major3 ~ 1, major4 ~ 1, major5 ~ 1, major6 ~ 1))
...
for (i in 1:7) {
  priors[[i]] <- lgcpPrior(etaprior = PriorSpec(LogGaussianPrior(mean = pr.mn, 
                 variance = diag(pr.vr))), betaprior = PriorSpec(GaussianPrior(mean = rep(0,8), variance = diag(10^6, 8))))
}
priors[[8]] <- lgcpPrior(etaprior = PriorSpec(LogGaussianPrior(mean = pr.mn, variance = diag(pr.vr))), betaprior = NULL)
...
cfs[[8]] <- CovFunction(RandomFieldsCovFct(model = "matern", additionalparameters = 1))
