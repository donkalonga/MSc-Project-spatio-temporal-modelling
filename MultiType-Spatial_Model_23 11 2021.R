...
source("lgcpPredictMultitypeSpatialPlusPars.DonKalonga.R")
...
dat$lineage[dat$lineage=="major4"] <- "major4_5"
dat$lineage[dat$lineage=="major5"] <- "major4_5" # combining major4 and major5 into a new lineage
dim(dat) # 255 cases
...
ttx <- xym$x[xym$marks == "major0" | xym$marks == "major1" | xym$marks == 
                   "major2" | xym$marks == "major3" | xym$marks == "major4_5" | xym$marks == "major6"]
...
form_list <- formulaList(list(major0 ~ 1, major1 ~ 1, major2 ~ 1, major3 ~ 1, major4_5 ~ 1, major6 ~ 1))
...
for (i in 1:5) {
  priors[[i]] <- lgcpPrior(etaprior = PriorSpec(LogGaussianPrior(mean = pr.mn, 
                  variance = diag(pr.vr))), betaprior = PriorSpec(GaussianPrior(mean = rep(0,6), variance = diag(10^6, 6))))
}
priors[[6]] <- lgcpPrior(etaprior = PriorSpec(LogGaussianPrior(mean = pr.mn, variance = diag(pr.vr))), betaprior = NULL)
...
cfs[[6]] <- CovFunction(RandomFieldsCovFct(model = "matern", additionalparameters = 1))
...
Multitype_Spatial_Model <- lgcpPredictMultitypeSpatialPlusPars.DonKalonga(formulaList = form_list, sd = xym, Zmat = Zmat, model.priorsList = priors, 
