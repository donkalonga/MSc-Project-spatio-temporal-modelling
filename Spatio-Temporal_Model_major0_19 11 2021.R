...
FORM <- X ~ Total_0.10 +  Ltt1 + Ltt2 + Ltt3
FORM_Spatial <- X ~ Total_0.10
FORM_Temporal <- t ~ Ltt1 + Ltt2 + Ltt3 -1

## SECOND MODEL FORMULAE
FORM_2 <- X ~ Ltt1 + Ltt2 + Ltt3
FORM_Spatial_2 <- X ~ 1
FORM_Temporal_2 <- t ~ Ltt1 + Ltt2 + Ltt3 -1
...
SpatioTemporal_Model_major0 <- lgcpPredictSpatioTemporalPlusPars(formula = FORM_2, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
                               model.inits = INITS, spatial.covmodel = CF, cellwidth = CellWidth, poisson.offset =  Pop.offset, 
                               mcmc.control = mcmcpars(mala.length = 1000000, burnin = 100000, retain = 90, adaptivescheme = andrieuthomsh(inith = 1, alpha = 0.5, C = 1, 
