...
dat<-read.csv("ST_Lineage_6_Data.csv") %>% # 132 cases
...
SpatioTemporal_major6 <- lgcpPredictSpatioTemporalPlusPars(formula = FORM_2, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
                             model.inits = INITS, spatial.covmodel = CF, cellwidth = CellWidth, poisson.offset =  Pop.offset, 
                             mcmc.control = mcmcpars(mala.length = 10000, burnin = 1000, retain = 10, adaptivescheme = andrieuthomsh(inith = 1, alpha = 0.5, C = 1, 
 ...
 save(list = ls(), file = file.path(DIRNAME, "ST_Models", "ST_Model_major6_output.RData")) 
 
# MODEL DIAGNOSTIC CHECKS by plotting the log target to check if the chain has converged to a posterior mode

plot(SpatioTemporal_major6)
plot(ltar(SpatioTemporal_major6), type = "s", xlab = "Iteration/900", ylab = "log target")
...
lagch <- c(1, 5, 15)
Sacf <- autocorr(SpatioTemporal_major6, lagch, inWindow = NULL)
library("fields")
for (i in 1:3) {
  image.plot(xvals(SpatioTemporal_major6), yvals(SpatioTemporal_major6), Sacf[, , i], zlim = c(-1, 1), axes = FALSE, 
...
traceplots(SpatioTemporal_major6, ask = FALSE)

# produce autocorrelation plots of beta and eta
parautocorr(SpatioTemporal_major6, ask = FALSE)

# a summary table of beta and eta
parsum <- parsummary(SpatioTemporal_major6)
...
textsummary(SpatioTemporal_major6, digits = 4)

# a plot of the prior and posterior densities
priorpost(SpatioTemporal_major6, ask = FALSE)

# the posterior covariance function
postcov(SpatioTemporal_major6, ask = FALSE)

# exceedance and lower-tail exceedance probabilities
ep <- exceedProbs(c(1.5, 3))
ex <- lgcp:::expectation.lgcpPredict(SpatioTemporal_major6, ep)

plotExceed(ex[[1]], "ep", SpatioTemporal_major6, zlim = c(0, 1), asp = 1, axes = FALSE, xlab = "", ylab = "", 
           sub = "", ask = FALSE)
scalebar(5000, label = "5 km")
