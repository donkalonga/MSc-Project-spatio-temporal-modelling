...
dat<-read.csv("ST_Lineage_2_Data.csv") %>% 
...
LTspline<-bs(tmu, knots = 14, df = 14)
bs1 <- LTspline[,1]
bs2 <- LTspline[,2]
bs3 <- LTspline[,3]
...
par(mfrow = c(4,4))
for (i in 1:15) {image.plot(ZmatList[[i]])}
...
INITS <- lgcpInits(etainit = log(c(sqrt(3.33), 235,1)), betainit = NULL)
...
SpatioTemporal_Model_major2 <- lgcpPredictSpatioTemporalPlusPars(formula = FORM, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
...
# plotting the model
plot(SpatioTemporal_Model_major2)
 
# plotting the log target to check if the chain has converged to a posterior mode
plot(ltar(SpatioTemporal_Model_major2), type = "s", xlab = "Iteration/900", ylab = "log target")
