...
dat<-read.csv("ST_Lineage_4_Data.csv") %>% # 132 cases
...
SpatioTemporal_major4 <- lgcpPredictSpatioTemporalPlusPars(formula = FORM_2, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
 ...
 save(list = ls(), file = file.path(DIRNAME, "ST_Models", "ST_Model_major4_output.RData")) 
 
# MODEL DIAGNOSTIC CHECKS by plotting the log target to check if the chain has converged to a posterior mode

plot(SpatioTemporal_major4)
plot(ltar(SpatioTemporal_major4), type = "s", xlab = "Iteration/900", ylab = "log target")
...
lagch <- c(1, 5, 15)
Sacf <- autocorr(SpatioTemporal_major4, lagch, inWindow = NULL)
library("fields")
for (i in 1:3) {
  image.plot(xvals(SpatioTemporal_major4), yvals(SpatioTemporal_major4), Sacf[, , i], zlim = c(-1, 1), axes = FALSE, 
...
traceplots(SpatioTemporal_major4, ask = FALSE)

# produce autocorrelation plots of beta and eta
parautocorr(SpatioTemporal_major4, ask = FALSE)

# a summary table of beta and eta
parsum <- parsummary(SpatioTemporal_major4)
...
textsummary(SpatioTemporal_major4, digits = 4)

# a plot of the prior and posterior densities
priorpost(SpatioTemporal_major4, ask = FALSE)

# the posterior covariance function
postcov(SpatioTemporal_major4, ask = FALSE)

# exceedance and lower-tail exceedance probabilities
ep <- exceedProbs(c(1.5, 3))
ex <- lgcp:::expectation.lgcpPredict(SpatioTemporal_major4, ep)

plotExceed(ex[[1]], "ep", SpatioTemporal_major4, zlim = c(0, 1), asp = 1, axes = FALSE, xlab = "", ylab = "", 
           sub = "", ask = FALSE)
scalebar(5000, label = "5 km")
