...
#head(splineTerms)
#datTemp <- data.frame(LTs1=LTspline[,1],LTs2=LTspline[,2],LTs3=LTspline[,3])
#head(datTempSum)
...
FORM <- X ~ Total_0.10 + moty + monthNum1 + monthNum2 + monthNum3
FORM_Spatial <- X ~ Total_0.10
FORM_Temporal <- t ~ moty + monthNum1 + monthNum2 + monthNum3 -1

## SECOND MODEL FORMULAE
FORM_2 <- X ~ moty + monthNum1 + monthNum2 + monthNum3
FORM_Spatial_2 <- X ~ 1
FORM_Temporal_2 <- t ~ moty + monthNum1 + monthNum2 + monthNum3 -1
...
tdata <- data.frame(t=tvecNum, moty= monthsOfTheYear, monthNum1 = LTspline[,1], monthNum2 = LTspline[,2], monthNum3 = LTspline[,3])
...
BetaP <- PriorSpec(GaussianPrior(mean = rep(0,15), variance = diag(1e+06,15)))
...
SpatioTemporal_Model_All_Cases <- lgcpPredictSpatioTemporalPlusPars(formula = FORM_2, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
 ...
 save(list = ls(), file = file.path(DIRNAME, "ST_Models", "ST_Model_All_Cases_output_LONG_TERM_TREND.RData")) 
