...
dat<-read.csv("ST_All-Lineages_Data.csv") %>% 
  dplyr::select(hh_LAT, hh_LNG, sample_collected, lineage) %>%
...
# selecting cases with the merged lineages only

dat <- dat[dat$lineage=='major13456',] %>% filter(!is.na(hh_LAT))
...
LTspline<-bs(tmu, knots = 16, df = 16)
bs1 <- LTspline[,1]
bs2 <- LTspline[,2]
bs3 <- LTspline[,3]
...
table(inside.owin(xy_utm$x,xy_utm$y,BTShapeF_owin))
...
# these approximates are used to inform size of the computational grid
...
par(mfrow = c(4,4))
for (i in 1:17) {image.plot(ZmatList[[i]])}
...
SpatioTemporal_Model_major13456 <- lgcpPredictSpatioTemporalPlusPars(formula = FORM, xyt = xyt, T = ti, laglength = LAGLENGTH, ZmatList = ZmatList, model.priors = priors, 
 ...
 save(list = ls(), file = file.path(DIRNAME, "ST_Models", "ST_Model_major13456_output.RData")) 
...
plot(ltar(SpatioTemporal_Model_major13456), type = "s", xlab = "Iteration/900", ylab = "log target")
